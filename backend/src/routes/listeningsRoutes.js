...
// Mapeia listings vindos da BD para o formato esperado pelo frontend.
function mapDbListings(dbListings) {
    return dbListings.map(dbItem => ({
...
async function parseAndProcessSteamData(steamData, marketHashName) {
    ...
    for (const el of $('.market_listing_row').toArray()) {
        ...
        if (inspectLink) {
            try {
                floatData = await extractFloatFromSteam(inspectLink);
                if (floatData.floatvalue) {
                    console.log(`✓ Float obtido direto do Steam para ${listingid}: ${floatData.floatvalue}`);
                }
            } catch (e) {
                console.warn(`⚠️ Erro ao extrair float do Steam: ${e.message}`);
            }
        }
...
router.get('/:marketHashName', async (req, res) => {
    ...
    try {
        ...
        if (!steamData || !steamData.success) {
            // Steam indisponível -> servir a partir da base de dados (fallback)
            ...
            if (itemId) {
                ...
                return res.json({
                    success: true,
                    marketHashName,
                    listings: mapDbListings(dbData.listings),
            ...
            return res.json({
                ...
                marketHashName,
                listings: [],
                pagination: { totalPages: 0, totalListings: 0 },
                source: 'unavailable'
            });
        }
        ...
        if (hasFilters) {
             ...
             if (itemId) {
                 ...
                 const mappedListings = mapDbListings(dbData.listings);
